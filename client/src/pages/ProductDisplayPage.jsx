...
const ProductDisplayPage = () => {
  ...
  return (
    <section className='container mx-auto p-4 grid lg:grid-cols-2  '>
      <div className=''>
        <div className='bg-white min-h-56 max-h-56 lg:min-h-[70vh] lg:max-h-[70vh] rounded h-full w-full '>
        ...
        <div className='flex items-center justify-center gap-3 my-2'>

          {
            data.image.map((img, index) => {
              return (
                <div key={img + index + 'point'} className={`bg-slate-200 w-3 h-3 lg:w-5 lg:h-5 rounded ${index === image && 'bg-slate-300'}`} ></div>
              )
            })
          }
        </div>
        <div className='grid relative'>
          <div ref={imageContainer} className='flex gap-3 w-full relative z-10 overflow-x-auto scrollbar-none'>
            {
              data.image.map((img, index) => {
                return (
                  <div className='w-20 h-20 min-h-20 min-w-20 shadow cursor-pointer' key={img + index}>
                    <img src={img}
                      alt='min-product=images'
                      onClick={() => {
                        setImage(index)
                      }}
                      className='w-full h-full object-scale-down' />
          ...
          <div className='w-full -ml-3 h-full flex justify-between absolute items-center'>
            <button className='bg-white p-1 relative rounded-full shadow-lg'
              onClick={handleScrollLeft}
            >
              <FaAngleLeft />
            </button>
            <button onClick={handleScrollRight} className='bg-white p-1 relative rounded-full shadow-lg'>
              <FaAngleRight />
            </button>
          </div>
        </div>
      ...
      <div className='p-4 lg:pl-7 text-base lg:text-lg'>
        ...
        <div>
          <div className='flex items-center gap-4 my-4'>
            <img src={image1} alt="Minute Delivery"
              className='w-20 h-20'
            />
          ...
          <div className='flex items-center gap-4 my-4'>
            <img src={image2} alt="Best prices offers"
              className='w-20 h-20'
            />
            <div className='text-sm'>
          ...
          <div className='flex items-center gap-4 my-4'>
            <img src={image3} alt="Wide assortment"
              className='w-20 h-20'
            />
            <div className='text-sm'>
              <div className='font-semibold' >Wide Assortment</div>
              <p>Choose from 5000+ products acress food personal care, household & other categories.</p>
            </div>
          </div>
        </div>
      </div>
